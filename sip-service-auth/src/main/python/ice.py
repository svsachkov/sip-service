...
def item_getter(path: str, file_name: str, transforms=transforms_val, val=False) -> (np.ndarray, np.ndarray):
    i_ = 'val_' if val else ''
    #image = np.load(f'images/inputs/img.tiff')
    image = rasterio.open('images/inputs/img.tiff', 'r').read()
...
def process(url):
    ...
    url1 = "https://services.sentinel-hub.com/ogc/wms/2cea12cc-7019-40a1-aa32-5948e8629ba9?REQUEST=GetMap&CRS=CRS:84" \
           "&BBOX=12.44693,41.870072,12.541001," \
           "41.917096&LAYERS=VV&WIDTH=512&HEIGHT=343&FORMAT=image/tiff&TIME=2023-02-27/2023-03-27"
    url2 = "https://services.sentinel-hub.com/ogc/wms/2cea12cc-7019-40a1-aa32-5948e8629ba9?REQUEST=GetMap&CRS=CRS:84" \
           "&BBOX=12.44693,41.870072,12.541001," \
           "41.917096&LAYERS=VH&WIDTH=512&HEIGHT=343&FORMAT=image/tiff&TIME=2023-02-27/2023-03-27"

    url3 = "https://services.sentinel-hub.com/ogc/wms/2cea12cc-7019-40a1-aa32-5948e8629ba9?REQUEST=GetMap&CRS=CRS:84" \
           "&BBOX=12.44693,41.870072,12.541001," \
           "41.917096&LAYERS=VV-20&WIDTH=512&HEIGHT=343&FORMAT=image/tiff&TIME=2023-02-27/2023-03-27"
    url4 = "https://services.sentinel-hub.com/ogc/wms/2cea12cc-7019-40a1-aa32-5948e8629ba9?REQUEST=GetMap&CRS=CRS:84" \
           "&BBOX=12.44693,41.870072,12.541001," \
           "41.917096&LAYERS=VH-20&WIDTH=512&HEIGHT=343&FORMAT=image/tiff&TIME=2023-02-27/2023-03-27"

    x1 = requests.get(url1)
    # Save input image:
    with open("images/inputs/1.tiff", 'wb') as f:
        f.write(x1.content)

    x2 = requests.get(url2)
    # Save input image:
    with open("images/inputs/2.tiff", 'wb') as f:
        f.write(x2.content)

    x3 = requests.get(url3)
    # Save input image:
    with open("images/inputs/3.tiff", 'wb') as f:
        f.write(x3.content)

    x4 = requests.get(url4)
    # Save input image:
    with open("images/inputs/4.tiff", 'wb') as f:
        f.write(x4.content)

    img1 = rasterio.open('images/inputs/1.tiff', 'r')
    img2 = rasterio.open('images/inputs/2.tiff', 'r')
    img3 = rasterio.open('images/inputs/3.tiff', 'r')
    img4 = rasterio.open('images/inputs/4.tiff', 'r')

    img = np.ndarray(shape=(4, *img1.shape))
    img[0] = np.array(img1.read(1))
    img[1] = np.array(img2.read(1))
    img[2] = np.array(img3.read(1))
    img[3] = np.array(img4.read(1))
    with rasterio.open("images/inputs/img.tiff", 'w', **img1.profile) as src:
        src.write(img)
    ...
    # ГОВОРИ, ЧТО МЕНЯТЬ -----------------------
    PIL.Image.fromarray(np.asarray(img).astype(np.uint8).transpose((1, 2, 0))).show()

    #img = np.load('images/inputs/' + input_img_path)
    ...
    dataset = InferDataset('images/inputs', os.listdir('images/inputs')[0])
